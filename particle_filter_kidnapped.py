...
def animate(controls, measurements, initial, landmarks_map, length, width, particles,estim_file):
    ...
    np.save(os.path.join("estim2", estim_file), estim_tr)
    print("created estim2/", estim_file)

# ------------------------------------------
def create_initial_particle(initial, n):
    particle = [[[random.uniform(0.3, 1.7),random.uniform(0.3, 1.7),random.uniform(0.0, 2 * math.pi)], 1 / n] for i in range(n)]
    return particle


# ------------------------------------------
if __name__ == "__main__":
    if not os.path.exists("estim2"):
        os.makedirs("estim2")
    ...
    estim_file = f"estim2_{args.sensing[18]}_{args.sensing[20]}_{args.sensing[22]}_{args.num_particles}"
